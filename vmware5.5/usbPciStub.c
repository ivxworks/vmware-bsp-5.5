...
/* MAX OHCI number to find */

#define MAX_USB_OHCI_NUM        8

/* USB OHCI pci memory size */

#define USB_OHCI_PCI_MEMSIZE    0x1000 

#ifndef OHCI_CLASS
#define OHCI_CLASS              0x0c
#endif  /* OHCI_CLASS */

#ifndef OHCI_SUBCLASS
#define OHCI_SUBCLASS           0x03
#endif  /* OHCI_SUBCLASS */

#ifndef OHCI_PGMIF
#define OHCI_PGMIF              0x10
#endif /* OHCI_PGMIF */
...
/***************************************************************************
*
* sysUsbOhciPciInit - Find the Ohci interfaces 
*
* This routine detects the Ohci interface, and add PCI memory base to the
* sysPhysMemDesc table.
* 
* RETURNS: N/A
*/

VOID sysUsbOhciPciInit (void)
    {
    int    index;         /* device index */
    UINT8  busNo;         /* PCI bus number */
    UINT8  devNo;         /* PCI device number */
    UINT8  funNo;         /* PCI function number */
    UINT32 memBar0;       /* PCI BAR0 value */

    for (index = 0; index < MAX_USB_OHCI_NUM; index++)
         {    
         if (usbPciClassFind (OHCI_CLASS, OHCI_SUBCLASS, OHCI_PGMIF, index,
                              &busNo, &devNo, &funNo) != TRUE)
             continue;

         memBar0 = usbPciDwordGet (busNo, devNo, funNo, PCI_CFG_BASE_ADDRESS_0);

         memBar0 &= PCI_MEMBASE_MASK;

         /* add the entry to sysPhysMemDesc[] */

#if (defined(INCLUDE_MMU_FULL) || defined(INCLUDE_MMU_BASIC))
         if (sysMmuMapAdd((void *)memBar0, USB_OHCI_PCI_MEMSIZE, 
